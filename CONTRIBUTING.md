...
- and of course, [the issues list](https://github.com/fortian/fxa-auth-db-postgres/issues)
...
2. The clone the upstream (as origin) and add your own repo as a remote:
    ...
    ```sh
    $ git clone https://github.com/fortian/fxa-auth-db-postgres.git
    $ cd fxa-auth-db-postgres
    $ git remote add user1 git@github.com:user1/fxa-auth-db-postgres.git
```
