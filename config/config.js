...
module.exports = function (fs, path, url, convict) {
  ...
  var conf = convict({
    ...
    master: {
      user: {
        doc: 'The user to connect to for PostgreSQL',
        default: 'fxadb',
        env: 'PGUSER',
      },
      password: {
        doc: 'The password to connect to for PostgreSQL',
        default: '',
        env: 'PGPASSWORD',
      },
      host: {
        doc: 'The host to connect to for PostgreSQL',
        default: '127.0.0.1',
        env: 'PGHOST',
      },
      port: {
        doc: 'The port to connect to for PostgreSQL',
        default: 5432,
        format: 'port',
        env: 'PGPORT',
      },
    ...
    slave: {
      ...
      connectionLimit: {
        ...
        default: 10,
        format: 'nat',
        env: 'PG_SLAVE_CONNECTION_LIMIT',
      },
      waitForConnections: {
        doc: "Determines the pool's action when no connections are available and the limit has been reached.",
        default: true,
        format: Boolean,
        env: 'PG_SLAVE_WAIT_FOR_CONNECTIONS',
      },
      queueLimit: {
        doc: "Determines the maximum size of the pool's waiting-for-connections queue.",
        default: 100,
        format: 'nat',
        env: 'PG_SLAVE_QUEUE_LIMIT',
      },
    },
    ipHmacKey: {
